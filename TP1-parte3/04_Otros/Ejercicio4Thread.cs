...
//Un hilo por tarea
//Simula 1000 tareas I/O-bound (sleep 100ms) y mide el tiempo total
class Program
{
    static void Main()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            hilos[i] = new Thread(() =>
            {
                Console.WriteLine($"Tarea {idx+1} ejecutada por {Thread.CurrentThread.ManagedThreadId}");
                Thread.Sleep(100); // Simula trabajo I/O-bound
                resultados[idx] = (idx + 1) * 10;
            });
            hilos[i].Start();
        }
        ...
        for (int i = 0; i < n; i++)
        {
            hilos[i].Join(); // Esperar que termine cada hilo
        }

        sw.Stop();
        Console.WriteLine($"Tiempo total con 1000 threads: {sw.Elapsed.TotalSeconds} segundos");
    }
}
