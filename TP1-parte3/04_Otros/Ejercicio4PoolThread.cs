...
class Program
{
    static void Main()
    {
        ...
        Parallel.For(0, n, i =>
        {
            Console.WriteLine($"Tarea {i + 1} ejecutada por {Thread.CurrentThread.ManagedThreadId}");
            Thread.Sleep(100); // Simula trabajo I/O-bound
            resultados[i] = (i + 1) * 10;
        });

        sw.Stop();
        Console.WriteLine($"Tiempo total con Parallel.For: {sw.Elapsed.TotalSeconds} segundos");
    }
}
